...
import {
  ...
  parseSalary, hasVisa, companyHue, companyColor, timeAgo, exactDate, deriveLevel,
} from "./derive.js";
...
export function statsFor(jobs) {
  const withSal = jobs.filter((j) => j.salMin);
  const salLo = withSal.length ? Math.min(...withSal.map((j) => j.salMin)) : 0;
  const salHi = withSal.length ? Math.max(...withSal.map((j) => j.salMax || j.salMin)) : 0;
